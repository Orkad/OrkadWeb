...
namespace OrkadWeb.Models
{
    ...
    public class UserShareMap : ClassMap<UserShare>
    {
        public UserShareMap()
        {
            ...
            HasMany(x => x.EmittedRefunds).KeyColumn("user_share_emitter_id").Inverse().Cascade.AllDeleteOrphan();
            HasMany(x => x.ReceivedRefunds).KeyColumn("user_share_receiver_id").Inverse().Cascade.AllDeleteOrphan();
        }
    }
}
